...
#define EXTERNAL_VOLTAGE 3300


#define ADC_VREF_INT25_OR_EXT (0)
#define ADC_VREF_YM_YP        (1)
#define ADC_VREF_XM_XP        (2)
#define ADC_VREF_INT25        (3)

#define ADC_USE_EXTERNAL_VREF (1)

#if ADC_USE_EXTERNAL_VREF == 1
#warning ADC_USE_EXTERNAL_VREF
#define ADC_VREF_VAL    (EXTERNAL_VOLTAGE)
#define ADC_VREF_SEL    (ADC_VREF_INT25_OR_EXT)
#else
#define ADC_VREF_VAL    (INTERNAL_VOLTAGE)
#define ADC_VREF_SEL    (ADC_VREF_INT25)
#endif
...
static void nuc970_detect2touch(void)
{
    ...
    __raw_writel(__raw_readl(REG_ADC_CONF)| ADC_CONF_TEN | ADC_CONF_ZEN | ADC_CONF_DISTMAVEN | ADC_CONF_DISZMAVEN | (1<<22) | (7<<3) | (ADC_VREF_SEL<<6), REG_ADC_CONF);
...
static int nuc970_ts_detect_down(struct nuc970_adc *nuc970_adc)
{
    ...
    {
        if((nuc970_adc->ts_state==TS_IDLE) && (nuc970_adc->isr & ADC_ISR_INTTC))
        {
...
static int nuc970ts_open(struct input_dev *dev)
{
    ...
    writel(__raw_readl(REG_ADC_CONF)  | (ADC_CONF_HSPEED|ADC_CONF_TEN | ADC_CONF_ZEN |  ADC_CONF_DISTMAVEN | (1<<22)|(1<<20) | (7<<3) | (ADC_VREF_SEL<<6)), REG_ADC_CONF); /* CONF */
...
static int nuc970adc_battery_read_adc(struct nuc970_adc *nuc970_adc)
{
    ...
    __raw_writel(__raw_readl(REG_ADC_CTL) | (ADC_CTL_ADEN ), REG_ADC_CTL);
#else
